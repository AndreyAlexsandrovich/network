...
export default async function apiProducts(place: HTMLElement) {
    ...
    try {
        ...
        json.products.forEach((item: Product) => {
            ...
            (templateProduct.querySelector("#title-product") as HTMLElement).textContent = productObject.title;
            (templateProduct.querySelector("#price-product") as HTMLElement).textContent = `$${productObject.price}`;
            (templateProduct.querySelector("#description") as HTMLElement).textContent = productObject.description;
            (templateProduct.querySelector("#image-product") as HTMLImageElement).src = productObject.image;
            (templateProduct.querySelector(".product") as HTMLElement).id = `product-${productObject.id}`;
         ...
         searchProduct(place, json.products)
    } catch (error) {
